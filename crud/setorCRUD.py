...
def adicionar_setor(nome_setor):
    setor_existe = Setor.filter(Setor.nome_setor == nome_setor)
...
def seleciona_nome_setores():
    setores = [ setor.nome_setor if setor is not None else '' for setor in Setor.select()]
    return setores

def selecionar_id_setor_por_nome(nome_setor):
    id_setor = Setor.get(Setor.nome_setor==nome_setor).id
